...
namespace CISystem
{
    public partial class ReviewEvidence : Form
    {
        public string SelectedCaseId { get; set; }
        public ReviewEvidence()
        {
            InitializeComponent();
            this.Load += new EventHandler(ReviewEvidence_Load);
        ...
        private void ReviewEvidence_Load(object sender, EventArgs e)
        {
        ...
        FROM Evidence
        WHERE case_id = @caseId AND status = 'Submitted'
        ";
                ...
                while (reader.Read())
                {
                    ...
                    Button btn_analysis = new Button { Text = "Accept", BackColor = Color.LightSkyBlue, ForeColor = Color.Black, Font = new Font("Arial", 9, FontStyle.Bold), FlatStyle = FlatStyle.Flat, Size = new Size(100, 30), Location = new Point(card.Width - 240, btn_view.Bottom + 10) };
                    Button btn_reject = new Button { Text = "Reject", BackColor = Color.Salmon, ForeColor = Color.Black, Font = new Font("Arial", 9, FontStyle.Bold), FlatStyle = FlatStyle.Flat, Size = new Size(100, 30), Location = new Point(card.Width - 125, btn_view.Bottom + 10) };

                    btn_analysis.FlatAppearance.BorderSize = 0;
                    btn_reject.FlatAppearance.BorderSize = 0;

                    btn_analysis.Click += (s, e) =>
                    {
                        string officerId = GetOfficerIdByUsername(UserSession.UserName);
                        if (officerId != null)
                            UpdateEvidenceStatus(evidenceId, "Accepted", officerId);
                        else
                    ...
                    btn_reject.Click += (s, e) =>
                    {
                        string officerId = GetOfficerIdByUsername(UserSession.UserName);
                        if (officerId != null)
                            UpdateEvidenceStatus(evidenceId, "Rejected", officerId);
                        else
                            MessageBox.Show("Officer not found.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    };
                    ...
                    card.Controls.AddRange(new Control[] {
                        ...
                        lbl_desc, val_desc,
                        btn_view, btn_analysis, btn_reject
                    });
                ...
                if (flp_evidence.Controls.Count == 0)
                {
                    ...
                    noLeadsLabel.Text = "No Submitted Evidence Found";
        ...
        private void UpdateEvidenceStatus(int evidenceId, string status, string officerId)
        {
            using (SqlConnection conn = new SqlConnection(DB.connectionString))
            {
                conn.Open();
        ...
        WHERE evidence_id = @evidenceId";
            ...
            MessageBox.Show($"Evidence marked as '{status}' successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
            LoadEvidence(); // Refresh the panel
        }
